	...
	char **argv;
{
    ...
    /* Tell when we start & stop printing */
int start_page = -1, end_page = 32000;
static void
set_range(arg)
    char *arg;
{
    ...
    if( isdigit(*p) ) start_page = atoi(p);
    while( isdigit(*p) ) ++p;
    if( *p != '-' ) return;
    end_page = atoi(p+1);
}
	...
	while((c = *++arg) != '\0')
		switch(c){
			...
			case 'e':	/* divert errors */
				close(2);
				if (open(++arg, O_WRITE|O_CREAT, 0600) < 0) {
					abort();
				}
				return;
			...
			case 'n':	/* set initial page number */
				...
				newpag = curpag;
				return;
			case 'o':	/* Choose page range to print */
			    set_range(++arg);
			    return;
